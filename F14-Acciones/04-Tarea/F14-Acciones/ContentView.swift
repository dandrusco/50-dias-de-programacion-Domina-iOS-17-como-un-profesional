...
struct ContentView: View {
            ...
            ForEach(self.courses){ course in
                SimpleImageRow(course: course)
                    .contextMenu {
                        ...
                        Button(action: {
                            //5. Implementamos la llamada setFavorite
                            self.setFavorite(course: course)
                        }, label: {
                            HStack{
                                //6. Modificaremos para que cuando el curso ya sea favorito nos diga eliminar de favoritos
    ...
    //2. Implementamos la funcion para favoritos
    private func setFavorite(course: Course) {
        if let index = self.courses.firstIndex(where: { $0.id == course.id}){
            //3. Con togle cambiamos el valor booleano cada vez que se precione
...
struct SimpleImageRow: View {
    ...
    var body: some View {
        HStack{
            ...
            //4. Píntaremos la estrella si es favorito
