...
struct walletView: View {
    ...
    private func zIndex(for card: Card) -> Double {
        ...
        //1. El zindex por defecto, se configura al opeuesto de la posicion que ocupa
        let defaultIndex = -Double(index)
        
        //2. Si estamos arrastrando la tarjeta
        if let dragIndex = self.dragState.index, index == dragIndex {
            //3. Caluclaremos la nueva Z segun la altura de la translacion
            return defaultIndex + Double(self.dragState.translation.height/walletView.cardOffset)
        }
        
        //3. Ahora debolveremos el valor por defecto
        /*return -Double(index)*/
        return defaultIndex
    }
