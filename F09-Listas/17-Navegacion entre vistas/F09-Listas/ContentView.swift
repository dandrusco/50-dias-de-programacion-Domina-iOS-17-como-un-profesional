...
//1. Crearemos un nuevo fichero de SwiftUI para la vista del detalle llamado CourseDetailView
        ...
        NavigationStack{
            List{
                ForEach(courses) { course in
                    //15. Ahora para que cada curso despliegue la vista del detalle deberemos meterlo en una NavigationLink, especificando en el destination la CourseDetalView y pasandole por parametro el curso en cuestion recuperado en el ForEach
                    NavigationLink(destination: CourseDetailView(course: course)){
                        //16. Por lo tanto lo cliqueable sera la SimpleImageRow
                        SimpleImageRow(course: course)
                        //17. Ahora si se fijan a parece una flecha en cada curso indicando que se puede navegar popr el 
                    }
                }
