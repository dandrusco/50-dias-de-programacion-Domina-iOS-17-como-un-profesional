...
struct ContentView: View {
    ...
    //1. Para mostrar la vista de SettingsView deberemos validarlo con un estado
    ...
    var body: some View {
        NavigationStack{
            ...
            .toolbar{
                //3. Pondremos un boton a la ToolbarItem alineandolo a la derecha
                ToolbarItem(placement: .topBarTrailing) {
                    Button(action: {
                        //4. Su Accion sera modificar el showSettings a verdadero al tocar este boton
                        showSettings = true
                    }, label: {
                        //5. Pondremos la imagen
                        Image(systemName: "gear.circle")
                            .font(.title)
                    })
                }
            }
            //6. A continuacion pondremos una Sheet que contendra una variable booleana isPresented que dependera del estado de la variable showSettings
            .sheet(isPresented: $showSettings) {
                //7. En caso de ser verdadero, llamaremos a la SettingsView()
                SettingsView()
                //8. AHora podemos apreciar que si entramos en el engranaje (ajustes) se despliega el menu para poder filtrar incluso con el color teal
            }
        }
        .tint(.teal)
    }
