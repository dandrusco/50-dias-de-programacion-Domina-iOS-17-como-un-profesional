...
struct ContentView: View {
    ...
    //1. Como vamos a marcar un curso como favorito al cliquearlo en la pantalla, la variable courses la pasaremos a crear como un estado
    @State var courses = [
    ...
    //2. Crearemos un segundo estado para poder marcar la seleccion del curso pero de tipo opcional
        ...
        NavigationStack{
            List{
                ForEach(courses) { course in
                    //3. De momento dejaremos la NavigationLink comentada para poder trabajar solo en la ventana principal
                    /*NavigationLink(destination: CourseDetailView(course: course)){*/
                        InfoImagenRow(course: course)
                        //13. Pondremos un onTapGesture para capturar lo que el cliente seleccione
                        .onTapGesture {
                            //14. Por lo tanto lo que el cliente marque, se lo pasaremos a la variable selectedCourse
...
//4. Renombraremos la SimpleImageRow por InfoImagenRow
    ...
    var body: some View {
        HStack{
            Image(course.image)
                ...
                //5. Eliminaremos este cornerRadius para utlizar otra tecnica
                /*.cornerRadius(25)*/
                //6. Ocuparemos el clipShape
                .clipShape(Circle())
                //7. Le pondremos un padding para que respire ya que le pondremos mas informacion
                .padding(.trailing, 16)
            
            //8. Inscrustaremos el texto en una VStack con alignment: .leading y luego en una HStack
            VStack(alignment: .leading) {
                HStack {
                    Text(course.name)
                        //9. Modificaremos la fuente del texto
                        .font(.system(.body, design: .rounded))
                        .bold()
                    //10. Pondremos una imagen
                    Image(systemName: "cellularbars")
                        .font(.subheadline)
                        .foregroundStyle(.teal)
                    //TODO: ajustar el numero de barra a la dificultad
                }
                //11. Justo aqui debajo pondremos el autor
                Text(course.author)
                    .font(.system(.footnote, design: .rounded))
                    .foregroundStyle(.secondary)
                
                //12. Mostraremos el valor
                Text("USD \(course.price)")
