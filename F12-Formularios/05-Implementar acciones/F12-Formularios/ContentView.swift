...
struct ContentView: View {
        ...
        NavigationStack{
            List{
                ForEach(courses) { course in
                    //8. Ya que lo hemos completado podemos volver a meter la navigacionlink (desconentarla) y ahora no solo podremos marcar como favorito, comprar o eliminar, si no que tambien podremos navegar por el menu
    ...
    private func purshase(item course: Course){
        //1. Vamos a localizar la posicion del curso dentro de la lista, para ello lo haremos con un casting seguro
        //2. En index localizaremos el cuso, pidiendole la primera aparicion (firstIndex) donde compararemos el identificador del curso con el del curso que estamos iterando
        if let index = courses.firstIndex(where: {$0.id == course.id}){
            //3. Si se da la condicion, osea que somos capaces de localizar la posicion del curso lo vamos a comprar pasando de false a true
            courses[index].isPurchased.toggle()
        }
    }

    private func favorite(item course: Course){
        //4. Para favorito es casi identico
        if let index = courses.firstIndex(where: {$0.id == course.id}){
    ...
    private func delete(item course: Course){
        //5. Para eliminar un curso es algo parecido
        if let index = courses.firstIndex(where: {$0.id == course.id}){
            //6. Una vez localizado lo que deberemos hacer es eliminar el curso
            courses.remove(at: index)
            //7. AHora ya podemos provar marcando un curso como comprado, como favorito o eliminarlo
        }
    }
}
            ...
            VStack(alignment: .leading) {
                HStack {
                    Text(course.name)
                        ...
                        .bold()
                    //9. Copiaremos este codigo para seguir con el mismo esquema y la pegaremos en la CourseDetailView
