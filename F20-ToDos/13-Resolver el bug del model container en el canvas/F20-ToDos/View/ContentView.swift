...
#Preview {
    ContentView()
    //7. AHora en esta preview lo inyectamos
        .modelContainer(previewContainer)
}

//1. Para solucionar errores en la preview realizaremos lo siguiente, crear un almacenamiento de memoria y rellenarlo con algunos datos
//2. Creamos un @MainActor para mostrar informacion en la ejecucion principal
@MainActor
//3. creamos un ModelCOntainer para asignar la variable
let previewContainer: ModelContainer = {
    do {
        let container = try ModelContainer(for: Task.self,
        configurations: ModelConfiguration(isStoredInMemoryOnly: true))
        
        //4. Creamoa datos de test
        for index in 1...5{
            let newTask = Task(title: "Tarea nº\(index)")
            container.mainContext.insert(newTask)
        }
        //5. devolvemos el contenedor
        return container
        // EN caso de error...
    } catch {
        fatalError("Error al crear el container")
    }
}()//7. Como es un clouster debemos abrir y cerrar parentesis
