...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack{
            List{
                //4. Nostros recorremos los cursos de manera ordenada, pues ahora le agregaremos el orden a travez de un .order y en DisplayOrder lo realizaremos por su tipo que es settings.displayOrder y el pediremos el sortPredicate
                ForEach(courses.sorted(by: DisplayOrder(type: self.settings.displayOrder).sortPredicate())) { course in
    ...
    private func shouldShow(course: Course) -> Bool {
        ...
        let favouriteCondition = !self.settings.showFavouriteOnly || (self.settings.showFavouriteOnly && course.isFavorite)
